...
//			if (DEBUG) Log.i(TAG, "unregister:");
	...
	//private boolean isUserChoosedPermission = false; //据此判断用户是否已操作 弹出的usb权限申请对话框
		...
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					try {
						//新增加的
						boolean isUserChoosedPermission = false; //据此判断用户是否已操作 弹出的usb权限申请对话框
						// パーミッションがなければ要求する
						mUsbManager.requestPermission(device, mPermissionIntent);
						//等待用户选择弹出的usb权限对话框
						int timeOutCount = 0;
						while(!isUserChoosedPermission && timeOutCount < 16){
							try {
								Thread.sleep(500);
